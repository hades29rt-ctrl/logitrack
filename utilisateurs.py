...
async def create_utilisateur(data: UtilisateurCreateSchema):
    ...
    try:
        existing = await conn.fetchrow(
            "SELECT id FROM logitrack.utilisateurs WHERE login = $1", data.login
        )
        if existing:
...
async def change_password(user_id: int, data: PasswordChangeSchema):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Mot de passe de {row['login']} mis a jour",
        }
    finally:
        await conn.close()
...
async def supprimer_utilisateur(user_id: int):
    ...
    try:
        user = await conn.fetchrow(
            "SELECT login, role FROM logitrack.utilisateurs WHERE id = $1", user_id
        )
        if not user:
            return {"error": "Utilisateur non trouve"}
        ...
        await conn.execute("DELETE FROM logitrack.utilisateurs WHERE id = $1", user_id)
...
async def enregistrer_flashage(data: FlashageSchema):
    ...
    try:
        ...
        if user["role"] in ("superviseur", "admin"):
            return {
                "success": True,
                "message": "Flashage non archive pour superviseur/admin",
            }
...
async def get_historique_flashage(
    user_id: int, date_debut: Optional[str] = None, date_fin: Optional[str] = None
):
    conn = await asyncpg.connect(**DB_CONFIG)
    try:
        query = """
