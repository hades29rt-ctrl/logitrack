...
BEGIN
    FOREACH allee IN ARRAY ARRAY['A','B','C'] LOOP
        FOR i IN 1..8 LOOP
            ...
            VALUES (allee || '-' || LPAD(i::TEXT, 2, '0'), allee, i, 1, 100)
            ON CONFLICT (code) DO NOTHING;
        END LOOP;
    END LOOP;
END;
...
SELECT
    'LOT-' || TO_CHAR(NOW() - INTERVAL '5 days', 'YYYYMMDD') || '-' || LPAD(rn::TEXT, 2, '0'),
    a.id,
    e.id,
    a.stock_actuel,
    NOW() - INTERVAL '5 days'
FROM (
    SELECT id, stock_actuel, ROW_NUMBER() OVER (ORDER BY id) AS rn
...
INSERT INTO lots (numero_lot, article_id, emplacement_id, quantite, date_peremption)
SELECT
    'LOT-PEREMPTION-TEST',
    (SELECT id FROM articles WHERE reference = 'ART-0231'),
    (SELECT id FROM emplacements WHERE code = 'C-01'),
    12,
    CURRENT_DATE + 3
ON CONFLICT (numero_lot) DO NOTHING;
...
INSERT INTO mouvements_stock
    (article_id, type_mvt, motif_sortie, quantite, stock_avant, stock_apres, reference_doc, operateur, scan_raw, created_at)
VALUES (
    (SELECT id FROM articles WHERE reference = 'ART-0055'),
    'sortie',
    'expedition',
    15,
    495, 480,
    'CMD-4421',
    'dupont.m',
    '(01)03700123456789(10)LOT-20241119-07(37)00015',
...
BEGIN
    ...
    RAISE NOTICE 'Alertes       : %', (SELECT COUNT(*) FROM logitrack.alertes_stock);
END;
$$;
